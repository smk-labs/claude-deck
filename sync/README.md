...
- **MCP servers.** The `mcpServers` block of every `claude_desktop_config.json` is reconciled. Add a server in one profile: it appears in every profile, `env` included, so it works there straight away. Edit a server's command or args in one profile: the edit propagates, and if two profiles disagree, the config edited most recently wins (key order alone is never a difference). **`env` is the exception, because that is where per-account credentials live.** A profile keeps its own values there forever; it only ever gains env keys it does not have yet. So two accounts can hold two different tokens for the same server without one overwriting the other, which is the entire point of separate profiles. The deliberate cost: changing an env value in one profile no longer spreads to the rest. Change it where you want it, or delete the key there and let the next sync refill it from another profile. Remove a server: the removal propagates, but **only with a witness**. `~/.claude/scripts/mcp-ledger.tsv` records which servers *each* config held at the end of the last sync, and a server is removed everywhere only when some config (a) is on record as having held it, (b) does not hold it now, (c) still holds at least one other server, and (d) did not lose several servers at once. That is the only state that means "you deleted it here". A profile that has never synced has no ledger rows and can never trigger a removal; a config the app reset to zero servers is excluded by (c); a config that lost two or more servers in one run is excluded by (d), and gets them all back on that same run. `--no-deletes` skips removals entirely and copies the missing servers back, which is the restore path for an accidental delete.
- **Server paths are checked against the disk.** A server whose `command` or `args` name a local absolute path is a note about where a file sits on this machine, not something a profile owns. So a definition whose paths all exist beats one whose paths do not, whatever the mtimes say. That is what stops the fix-then-break loop: when a plugin moves its server file and repairs one config, that repair now wins everywhere instead of being overwritten by the eleven stale copies on the next run. If NO config can resolve the path, the server is left exactly as it is in every config (one `MCP path skip` line, nothing written, and nothing added where it is missing) until the file comes back. Bare commands resolved through PATH, URLs, and anything holding `${VAR}` are never judged. Out-of-band repair, including for the app's own live config: `claude-deck mcp-doctor --fix`.
...
Nothing deletes an `env` key either: a profile's own credentials are never removed by a sync, only added to.
...
**An MCP server stopped working after a plugin update, and fixing it never sticks.**
The plugin moved its server file, so every profile config still points at the old path. Run `claude-deck mcp-doctor` to see which entries name a file that is gone, and `claude-deck mcp-doctor --fix` to repair them from whichever config already has the working path. A sync no longer undoes that repair: a definition whose paths exist now outranks a newer one whose paths do not.
